...
namespace Erlang.NET
{
	 ...
	 * @param flags
	 */
	public OtpInputStream(byte[] buf, int flags) : base(buf)
	{
	    this.flags = flags;
	}
	 ...
	 * @param flags
	 */
	public OtpInputStream(byte[] buf, int offset, int length, int flags) : base(buf, offset, length)
	{
	    this.flags = flags;
	}
	...
	public bool read_boolean()
	{
	    try
	    {
		return Boolean.Parse(read_atom());
	    }
	...
	public OtpErlangPort read_port()
	{
		...
		throw new OtpErlangDecodeException("Wrong tag encountered, expected " + OtpExternal.portTag
						   + ", got " + tag);
	    }
	...
	public OtpErlangExternalFun read_external_fun()
	{
	    ...
	    String module = read_atom();
	    String function = read_atom();
	    int arity = (int) read_long();
