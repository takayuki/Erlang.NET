...
namespace Erlang.NET
{
    internal class OtpMD5
    {
        ...
        private readonly long[] state = { 0x67452301L, 0xefcdab89L, 0x98badcfeL, 0x10325476L };
        private readonly long[] count = { 0L, 0L };
        ...
        {
            int index = (int)(((ulong)count[0]) >> 3 & 0x3F);
        ...
        {
            System.Console.WriteLine("state = {" + state[0] + ", " + state[1] + ", " + state[2] + ", " + state[3] + "}");
            System.Console.WriteLine("count = {" + count[0] + ", " + count[1] + "}");
        ...
        private void decode(long[] output, int[] input, int in_from, int len)
        {
            ...
            {
                output[i] = (long)((ulong)input[j + in_from]
                         | (ulong)shl(input[j + in_from + 1], 8)
                         | (ulong)shl(input[j + in_from + 2], 16)
                         | (ulong)shl(input[j + in_from + 3], 24));
            }
        }
        ...
        private int[] encode(long[] input, int len)
        {
            ...
            for (i = 0, j = 0; j < len; i++, j += 4)
            {
                ...
                output[j + 1] = (int)(((ulong)input[i]) >> 8 & 0xff);
                output[j + 2] = (int)(((ulong)input[i]) >> 16 & 0xff);
                output[j + 3] = (int)(((ulong)input[i]) >> 24 & 0xff);
            }
            return output;
        }
        ...
        public int[] final_bytes()
        {
            ...
            index = (int)(((ulong)count[0]) >> 3 & 0x3f);
